...
const uploadImage = ( url_upload_image, token ) => {
    ...
    let btnClass = $('.btnFileImage');

    let btnFile  = $('.btnFileImage :file');
...
const uploadVideo = ( url_upload_video, token ) => {

    let btnClass = $('.btnFileVideo');

    let btnFile  = $('.btnFileVideo :file');
    ...
    btnFile.on('fileselect',function(event,numFiles,label){
        ...
        let files        = $('#upload_video')[0].files[0];

        let action       = $(btnClass).data('action');

        let inputUrl     = $(btnClass).data('input-url');

        let previewVideo = $(btnClass).data('preview-video');
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $('#' + previewVideo).attr('src', response.location).trigger('change');
